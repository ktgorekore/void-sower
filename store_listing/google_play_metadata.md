<!--
  Copyright 2026 Void Sower Authors.

  Licensed under the Apache License, Version 2.0 (the "License");
  you may not use this file except in compliance with the License.
  You may obtain a copy of the License at

      http://www.apache.org/licenses/LICENSE-2.0

  Unless required by applicable law or agreed to in writing, software
  distributed under the License is distributed on an "AS IS" BASIS,
  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
  See the License for the specific language governing permissions and
  limitations under the License.
-->

# Google Play Store Listing Metadata & Localized Copy

This document defines the official production store listing copy, keywords, localized metadata, and asset specifications for **Void Sower: Kinetic Mancala** (`com.voidsower.app`).

---

## 🇺🇸 English (en-US) — Default

### App Title (Max 30 characters)
`Void Sower: Kinetic Mancala`

### Short Description (Max 80 characters)
`Afrofuturist orbital dreadnought defense powered by kinetic count-and-capture.`

### Full Description (Max 4,000 characters)
```text
Command the ultimate orbital dreadnought and defend the Kilwa Nebula Basin in Void Sower: Kinetic Mancala—a high-speed tactical arcade strategy game that reimagines the ancient Swahili board game Bao la Kiswahili into an energetic interstellar combat system!

Defend the frontier against waves of descending alien assault craft. Instead of firing conventional turrets, your flagship's weapons system is driven by a 16-bay circular capacitor ring. Harness ancient mathematical counting laws, inject reserve plasma cores, and sow energetic charges along circular trajectories to unleash devastating axial Particle Lances!

⚔️ REVOLUTIONARY KINETIC COMBAT
• Count-and-Capture Tactics: Inject plasma cores (Namua) into capacitor bays and swipe to sow charges along the ring in clockwise or counter-clockwise cadence.
• 28-Core Finite Economy: Manage your dreadnought's finite 28-core reactor fuel—every Namua injection draws from reserve power to initiate strategic cascades.
• Quadratic Particle Lances: When your sowing sequence terminates in an occupied frontline bay, its accumulated mass discharges as a devastating laser lance. Damage scales quadratically: D = α · M²!
• Cascade Relays & Conduit Shields: Trigger multi-lap chain reactions across the ring and deflect incoming enemy ordnance with charged frontline batteries.

🌌 THE KILWA NEBULA CAMPAIGN
• 7 Contested Star Sectors: Battle across the Outer Bastions, navigate the turbulent Monsoon Straits, and defend the vital Core Siphon.
• Dynamic Enemy Swarms: Outmaneuver scout interceptors, heavy gunships, and fortified enemy flagships with frequency-matched shielding.
• Tactical Dreadnought Chassis: Equip and master the balanced MK-I Bastion, the high-capacity MK-II Monsoon Vanguard, and the devastating MK-III Singularity Sovereign.

🚀 FLIGHT ACADEMY & INTUITIVE ONBOARDING
• 5-Step Interactive Flight Academy: Master 28-core reactor fuel, Namua injection, sowing cadences, quadratic lances, and lateral platform positioning with interactive tutorials.
• Two-Strip Tactical HUD: Monitor reactor fuel, sector threat tiers, live guidance beacons, and one-tap AI Tactical Solver controls in real time.
• Holographic Aiming Telemetry: Inspect real-time damage projections, shield-break probabilities, and corridor trajectories on the tactical Projection Shelf.
• In-Game Bao Codex: Discover the rich mathematical heritage and cultural lore behind traditional East African count-and-capture games.

💎 AFROFUTURIST AUDIOVISUAL SPLENDOR
• Custom GLSL Fragment Shaders: Experience pulsing plasma capacitors, chromatic shockwave bursts, and energetic scrolling horizons.
• Low-Latency SoundPool Audio: Harmonic pitch-ramping chimes celebrate multi-lap cascade relays with tactile haptic synchronization.
• 100% Native C++ Engine: Zero-allocation game simulation powered by EnTT ECS, cache-aligned ring buffers, and Android 15 16 KB page size optimization.

Join the fleet, master the sacred cadence, and liberate the stars!
```

---

## 🇹🇿 Swahili (sw)

### Jina la Programu (Title)
`Void Sower: Mancala ya Anga`

### Maelezo Mafupi (Short Description)
`Ulinzi wa manowari za angani unaoendeshwa na mbinu za mchezo wa asili wa Bao.`

### Maelezo Kamili (Full Description)
```text
Ongoza manowari kubwa ya anga na linda Ukanda wa Kilwa Nebula katika Void Sower: Mancala ya Anga—mchezo wa kusisimua wa mikakati unaobadilisha mchezo wa asili wa Kiafrika wa Bao la Kiswahili kuwa vita vya nyota!

Kinga mipaka dhidi ya makundi ya meli za adui zinazoshambulia. Mfumo wa silaha wa meli yako unaendeshwa na pete ya vituo 16 vya nishati. Tumia kanuni za hesabu za kale, weka chembe za plazma (Namua), na panda nishati kwenye mzunguko ili kurusha miale mikali ya Particle Lance!

⚔️ VITA VYA KISASA VYA MANCALA
• Mbinu za Bao la Kiswahili: Weka nishati (Namua) kwenye vyumba vya manowari na telezesha kidole ili kupanda nishati mzungukoni.
• Miale ya Nguvu (D = α · M²): Wakati upandaji unapokamilika kwenye chumba chenye nishati, mualiko mkali wa leza unarushwa kupitia njia ya vita!
• Msururu wa Ushindi (Takasika): Washa miitikio ya mfululizo inayofagia njia kadhaa za adui kwa hatua moja thabiti.

🌌 SAFARI YA KILWA NEBULA
• Maeneo 7 ya Vita: Pigana kwenye Ngome za Nje, vuka Njia ya Monsuni, na linda Chemchemi Kuu ya Kilwa Kisiwani.
• Aina za Manowari: Fungua na uendeshe MK-I Bastion, MK-II Monsoon, na MK-III Singularity Sovereign.
• Chuo cha Mafunzo ya Anga: Jifunze hatua kwa hatua kupitia mafunzo ya mwingiliano na mwongozo wa kina wa Bao Codex.

Jiunge na jeshi la anga na ukomboe nyota sasa!
```

---

## 🇳🇬 Yoruba (yo)

### Orukọ Ohun Elo (Title)
`Void Sower: Ogun Ayò Ayé`

### Apejuwe Kukuru (Short Description)
`Idaabobo ọkọ ofurufu ti a ṣe pẹlu awọn ilana iṣiro Ayò Olópon ti ilẹ Afirika.`

### Apejuwe Kikun (Full Description)
```text
Ṣe olori ọkọ oju-ofurufu nla ki o daabobo Kilwa Nebula Basin ninu Void Sower: Ogun Ayò Ayé—ere igbese ti o yi ere Ayò Olópon pada si eto ogun laarin awọn irawọ!

Dabobo agbegbe rẹ lodi si awọn ọta ti n sọkalẹ. Eto ohun ija ọkọ rẹ nlo oruka capacitor ti o ni ihò 16. Lo awọn ofin iṣiro atijọ, fi agbara plasma (Namua) sinu awọn ihò, ki o si gbin agbara lati ta awọn ina Particle Lance ti o lagbara pupọ!

⚔️ OGUN KINETIC TI AYÒ
• Ilana Gbingbin Ayò: Fi agbara sinu awọn ihò ki o ra ika rẹ lati gbin agbara ni ọna ti o yika.
• Ina Alagbara (D = α · M²): Nigbati gbingbin ba pari sinu ihò ti o ni agbara, ina nla yoo ta lati pa awọn ọta run!
• Ikẹkọ Ọkọ Ofurufu: Kọ ẹkọ ni igbesẹ nipasẹ igbesẹ pẹlu awọn itọsọna ibanisọrọ ati Bao Codex.

Darapọ mọ ogun ki o gba awọn irawọ la!
```

---

## 🇫🇷 French (fr-FR)

### Titre de l'application (Title)
`Void Sower: Mancala Cinétique`

### Brève description (Short Description)
`Défense orbitale afrofuturiste propulsée par le jeu de semis traditionnel Bao.`

### Description Complète (Full Description)
```text
Prenez les commandes du cuirassé orbital ultime et défendez le Bassin de la Nébuleuse de Kilwa dans Void Sower: Mancala Cinétique—un jeu d'arcade tactique qui réinvente le jeu ancestral d'Afrique de l'Est Bao la Kiswahili en un système de combat stellaire à haute énergie!

Défendez la frontière spatiale contre des vagues d'assaut extraterrestres. Le système d'armement de votre vaisseau amiral est alimenté par un anneau de 16 condensateurs plasma. Injectez des cœurs d'énergie (Namua) et semez les charges le long des trajectoires circulaires pour déclencher de redoutables Lances de Particules axiales dont les dégâts évoluent de façon quadratique: D = α · M²!

CARACTÉRISTIQUES PRINCIPALES :
• Mécanique de Semis & Capture : Tactiques de mancala réinventées avec cascades multi-tours dévastatrices.
• Campagne Kilwa Nebula : 7 secteurs stellaires progressifs avec modificateurs dynamiques et boss imposants.
• Académie de Vol Interactive : Tutoriel complet pour maîtriser les bases du Bao et la visée holographique.
• Moteur C++ Natif Haute Performance : Rendu Impeller 60/120 FPS fluide avec shaders GLSL personnalisés.
```

---

## 🇪🇸 Spanish (es-ES)

### Título de la aplicación (Title)
`Void Sower: Mancala Cinético`

### Breve descripción (Short Description)
`Defensa orbital afrofuturista impulsada por la mecánica tradicional del Bao.`

### Descripción Completa (Full Description)
```text
¡Ponte al mando del dreadnought orbital definitivo y defiende la Cuenca de la Nebulosa Kilwa en Void Sower: Mancala Cinético! Un juego táctico de acción espacial que reimagina el juego tradicional africano Bao la Kiswahili como un electrizante sistema de combate intergaláctico.

Siembre cargas de plasma a lo largo de un anillo de 16 condensadores axiales y descargue devastadoras Lanzas de Partículas con daño cuadrático: D = α · M².

CARACTERÍSTICAS PRINCIPALES:
• Táctica de Siembra y Captura: Domine las trayectorias de energía circular para abatir enjambres enemigos.
• Campaña Nebulosa de Kilwa: 7 sectores disputados con naves de asalto y acorazados fuertemente blindados.
• Academia de Vuelo Interactiva: Guía paso a paso para aprender las reglas y tácticas del Bao.
• Motor Nativo C++ Ultrarrápido: Simulación ECS a 60/120 FPS optimizada para Android 15.
```

---

## 🎥 Google Play Promo Video Deliverables

| Asset | Local File Path | Specifications | Purpose |
| :--- | :--- | :--- | :--- |
| **60s Narrated Tutorial Video** | [`store_listing/assets/how_to_play_60s.mp4`](assets/how_to_play_60s.mp4) | 1080x2400 Portrait, 60s, Neural voiceover (`en-US-ChristopherNeural`), HUD subtitles, ambient synth score | Primary YouTube / Play Store trailer explaining rules, controls, and combat |
| **30s Solver Showcase Video** | [`store_listing/assets/promo_gameplay.mp4`](assets/promo_gameplay.mp4) | 1080x2400 Portrait, 30s, High-action AI tactical solver, multi-tier sector clears, synth audio | High-CTR short showcase for YouTube Shorts and mobile store previews |
| **Animated Promo Gameplay GIF** | [`store_listing/assets/promo_gameplay.gif`](assets/promo_gameplay.gif) | 360x800, 12s loop, optimized 128-color Bayer dither, 1.79 MB | Lightweight animated preview for developer portals, GitHub READMEs, and store cards |
